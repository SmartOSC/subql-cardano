...
import {AuthToken, createHash32, generateTokenName, getIdFromTokenName} from './utils';
import {hexToBytes} from './hex';
// import * as blake2b from '@types/blake2b';
...
export function extractTxOutput(txBabbageBodies: BabbageTransactionBodyList): TxOutput[] {
  ...
  for (let idx = 0; idx < txBabbageBodies.len(); idx++) {
    ...
    for (let subIdx = 0; subIdx < txOutputs.len(); subIdx++) {
      ...
      switch (kind) {
        case 0: // Alonzo
          const alonzoTxOut = output.as_alonzo_format_tx_out() as AlonzoFormatTxOut;
          ...
          break;
        case 1:
          const babbageTxOut = output.as_babbage_format_tx_out() as BabbageFormatTxOut;
          outputs.push({
            ...
            datum_plutus: babbageTxOut.datum_option()?.as_datum() ?? new PlutusData(),
...
export function extractMultiAssets(multiAsset: MultiAsset): Map<string, TokenAsset[]> {
...
export function getTransactionHashFromTxBody(txBody: BabbageTransactionBody): string {
  return generateTransactionHash(txBody.to_cbor_bytes(), new Uint8Array());
}

export function generateTransactionHash(data: Uint8Array, prefix: Uint8Array): string {
  const bytes = data;
  if (!prefix.length) {
    bytes.set(prefix);
  }
...
export function hasTokenPrefix(assets: Map<string, TokenAsset[]>, tokenPrefix: string): boolean {
